...
final class CalendarUtility {
    ...
    func endOfWeek(for date: Date) -> Date? {
        ...
        return date(byAdding: .day, value: 6, to: startOfWeek)
    }
    ...
    func datesInWeek(startingFrom date: Date) -> [Date] {
        ...
        return (0..<7).compactMap { dayOffset in
            date(byAdding: .day, value: dayOffset, to: startOfWeek)
        }
    }
    ...
    /// Formats time components to a readable string
...
extension Date {
    ...
    var calendar: CalendarUtility {
        CalendarUtility.shared
    }
}
...
// Use safe operations:
let start = CalendarUtility.shared.oneMonthAgo ?? Date()

// Instead of:
let currentWeekStart = calendar.date(from: calendar.dateComponents([.yearForWeekOfYear, .weekOfYear], from: currentDate))!

// Use:
let currentWeekStart = CalendarUtility.shared.startOfWeek(for: currentDate) ?? currentDate
...
// Use:
let twoDaysAgo = CalendarUtility.shared.hoursAgo(userData.settings.muscleRestDuration) ?? Date()
