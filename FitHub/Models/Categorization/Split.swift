...
extension SplitCategory {
    static let upperBody: Set<SplitCategory> = [.abs, .back, .shoulders, .arms, .chest, .biceps, .triceps, .forearms]
    ...
    static func concatenateCategories(for categories: [SplitCategory]) -> String {
        ...
        if categories.contains(.legs) {
            if !legSubcats.isEmpty {
                ...
                parts.append("Legs") // .legs alone, no finer subcats → just “Legs”
            }
        }
        ...
        if !categories.contains(.legs) {
            parts = categories.map { $0.rawValue }  // Just show everything by rawValue
        } else {
            // .legs was in categories, so append any “other” (non‐leg) categories
            let otherNames = others
                .filter { $0 != .legs } // avoid re‐adding “.legs” itself
                .map { $0.rawValue }
