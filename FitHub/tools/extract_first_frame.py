#!/usr/bin/env python3
"""
Extract the first frame from each GIF in fithub_gifs/ and save as PNG in Exercise_PNG(f1)/
No cropping - preserves original dimensions
"""
...
def extract_first_frame(gif_path: Path, output_dir: Path) -> bool:
    """
    Extract the first frame from a GIF and save it as a PNG.
    ...
    try:
        ...
        with Image.open(gif_path) as img:
            ...
            # Create output filename: original name + "(f1)" + .png
            ...
            print(f"✓ Extracted: {gif_path.name} → {output_filename} ({frame_count} frames, used frame 0)")
            return True
...
def main():
    ...
    output_dir = script_dir / 'Exercise_PNG(f1)'
    ...
    for gif_path in gif_files:
        if extract_first_frame(gif_path, output_dir):
            success_count += 1
        else:
            error_count += 1
